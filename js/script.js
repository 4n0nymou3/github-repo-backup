async function fetchAllRepositories(username, page = 1, allRepos = []) {
  try {
    ...
    return repos.length === 100 ? fetchAllRepositories(username, page + 1, combinedRepos) : combinedRepos;
  } catch (error) {
    throw error;
  }
}
...
async function checkRepositories() {
  ...
  try {
    ...
    if (!userResponse.ok) {
      if (userResponse.status === 404) throw new Error('User not found. Please check the username and try again.');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('check-button').addEventListener('click', checkRepositories);
  document.getElementById('clear-button').addEventListener('click', resetEverything);
  ...
  searchInput.addEventListener('input', function() {
    ...
    repoItems.forEach(item => {
      ...
      item.style.display = (repoName.includes(searchTerm) || repoDesc.includes(searchTerm)) ? 'flex' : 'none';
    });
  });
});